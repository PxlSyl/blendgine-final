...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        let overlay_r = overlay_channel(dst_r, src_r);
        let overlay_g = overlay_channel(dst_g, src_g);
        let overlay_b = overlay_channel(dst_b, src_b);
        
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(overlay_r, overlay_g, overlay_b, overlay_alpha));
        return;
    }
    ...
    let overlay_r_premult = overlay_r * overlay_alpha;
    let overlay_g_premult = overlay_g * overlay_alpha;
    let overlay_b_premult = overlay_b * overlay_alpha;
    
    let inv_source_a = 1.0 - overlay_alpha;
    
    let result_r = overlay_r_premult + base.r * inv_source_a;
    let result_g = overlay_g_premult + base.g * inv_source_a;
    let result_b = overlay_b_premult + base.b * inv_source_a;
