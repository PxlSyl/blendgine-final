...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(multiply_r, multiply_g, multiply_b, overlay_alpha));
        return;
    }
    ...
    let multiply_r_premult = multiply_r * overlay_alpha;
    let multiply_g_premult = multiply_g * overlay_alpha;
    let multiply_b_premult = multiply_b * overlay_alpha;
