...
fn color_burn_channel(source: f32, destination: f32) -> f32 {
    ...
    return max(0.0, 1.0 - (1.0 - destination) / source);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        let burn_r = color_burn_channel(src_r, dst_r);
        let burn_g = color_burn_channel(src_g, dst_g);
        let burn_b = color_burn_channel(src_b, dst_b);
        
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(burn_r, burn_g, burn_b, overlay_alpha));
        return;
    }
    ...
    let burn_r_premult = burn_r * overlay_alpha;
    let burn_g_premult = burn_g * overlay_alpha;
    let burn_b_premult = burn_b * overlay_alpha;
    
    let inv_source_a = 1.0 - overlay_alpha;
    
    let result_r = burn_r_premult + base.r * inv_source_a;
    let result_g = burn_g_premult + base.g * inv_source_a;
    let result_b = burn_b_premult + base.b * inv_source_a;
