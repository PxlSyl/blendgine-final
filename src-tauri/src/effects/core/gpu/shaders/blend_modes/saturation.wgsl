...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        let blended_hsl = vec3<f32>(dst_hsl.x, src_hsl.y, dst_hsl.z);
        let blended_rgb = hsl_to_rgb(blended_hsl);
        
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(blended_rgb, overlay_alpha));
        return;
    }
    ...
    let blend_r_premult = blended_rgb.r * overlay_alpha;
    let blend_g_premult = blended_rgb.g * overlay_alpha;
    let blend_b_premult = blended_rgb.b * overlay_alpha;
