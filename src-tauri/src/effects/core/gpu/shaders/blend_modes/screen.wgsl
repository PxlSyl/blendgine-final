...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(screen_r, screen_g, screen_b, overlay_alpha));
        return;
    }
    ...
    let screen_r_premult = screen_r * overlay_alpha;
    let screen_g_premult = screen_g * overlay_alpha;
    let screen_b_premult = screen_b * overlay_alpha;
    
    let inv_source_a = 1.0 - overlay_alpha;
    
    let result_r = screen_r_premult + base.r * inv_source_a;
    let result_g = screen_g_premult + base.g * inv_source_a;
    let result_b = screen_b_premult + base.b * inv_source_a;
