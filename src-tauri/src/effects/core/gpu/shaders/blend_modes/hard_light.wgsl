...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (overlay_alpha >= 0.9999) {
        ...
        let hard_light_r = hard_light_channel(dst_r, src_r);
        let hard_light_g = hard_light_channel(dst_g, src_g);
        let hard_light_b = hard_light_channel(dst_b, src_b);
        
        textureStore(output_texture, vec2<u32>(pos), vec4<f32>(hard_light_r, hard_light_g, hard_light_b, overlay_alpha));
        return;
    }
    ...
    let hard_light_r_premult = hard_light_r * overlay_alpha;
    let hard_light_g_premult = hard_light_g * overlay_alpha;
    let hard_light_b_premult = hard_light_b * overlay_alpha;
    
    let inv_source_a = 1.0 - overlay_alpha;
    
    let result_r = hard_light_r_premult + base.r * inv_source_a;
    let result_g = hard_light_g_premult + base.g * inv_source_a;
    let result_b = hard_light_b_premult + base.b * inv_source_a;
