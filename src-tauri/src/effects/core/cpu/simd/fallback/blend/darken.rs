use super::base::{BlendProcessor, BlendResult, PixelData};

pub struct DarkenBlend;

impl DarkenBlend {
    pub fn new() -> Self {
        Self
    }
}

impl BlendProcessor for DarkenBlend {
    fn blend_pixels(&self, source: &PixelData, destination: &PixelData) -> BlendResult {
        if source.a <= 0.0001 {
            return BlendResult {
                pixel: *destination,
            };
        }

        if source.a >= 0.9999 {
            let src_r = source.r / source.a;
            let src_g = source.g / source.a;
            let src_b = source.b / source.a;

            let dst_r = if destination.a > 0.0001 {
                destination.r / destination.a
            } else {
                0.0
            };
            let dst_g = if destination.a > 0.0001 {
                destination.g / destination.a
            } else {
                0.0
            };
            let dst_b = if destination.a > 0.0001 {
                destination.b / destination.a
            } else {
                0.0
            };

            let darken_r = src_r.min(dst_r);
            let darken_g = src_g.min(dst_g);
            let darken_b = src_b.min(dst_b);

            return BlendResult {
                pixel: PixelData::from_unpremultiplied_normalized(
                    darken_r, darken_g, darken_b, source.a,
                ),
            };
        }

        if destination.a <= 0.0001 {
            return BlendResult { pixel: *source };
        }

        let src_r = if source.a > 0.0001 {
            source.r / source.a
        } else {
            0.0
        };
        let src_g = if source.a > 0.0001 {
            source.g / source.a
        } else {
            0.0
        };
        let src_b = if source.a > 0.0001 {
            source.b / source.a
        } else {
            0.0
        };

        let dst_r = if destination.a > 0.0001 {
            destination.r / destination.a
        } else {
            0.0
        };
        let dst_g = if destination.a > 0.0001 {
            destination.g / destination.a
        } else {
            0.0
        };
        let dst_b = if destination.a > 0.0001 {
            destination.b / destination.a
        } else {
            0.0
        };

        let darken_r = src_r.min(dst_r);
        let darken_g = src_g.min(dst_g);
        let darken_b = src_b.min(dst_b);

        let darken_r_premult = darken_r * source.a;
        let darken_g_premult = darken_g * source.a;
        let darken_b_premult = darken_b * source.a;

        let inv_source_a = 1.0 - source.a;

        let result_r = darken_r_premult + destination.r * inv_source_a;
        let result_g = darken_g_premult + destination.g * inv_source_a;
        let result_b = darken_b_premult + destination.b * inv_source_a;

        let result_a = source.a + destination.a * inv_source_a;

        let (final_r, final_g, final_b) = if result_a > 0.0001 {
            (
                result_r / result_a,
                result_g / result_a,
                result_b / result_a,
            )
        } else {
            (0.0, 0.0, 0.0)
        };

        BlendResult {
            pixel: PixelData::from_normalized(final_r, final_g, final_b, result_a),
        }
    }
}
