...
impl BlendProcessor for LightenBlend {
    fn blend_pixels(&self, source: &PixelData, destination: &PixelData) -> BlendResult {
        ...
        if source.a >= 0.9999 {
            ...
            let lighten_r = src_r.max(dst_r);
            let lighten_g = src_g.max(dst_g);
            let lighten_b = src_b.max(dst_b);

            return BlendResult {
                pixel: PixelData::from_unpremultiplied_normalized(
                    lighten_r, lighten_g, lighten_b, source.a,
                ),
            };
        }
        ...
        let lighten_r_premult = lighten_r * source.a;
        let lighten_g_premult = lighten_g * source.a;
        let lighten_b_premult = lighten_b * source.a;

        let inv_source_a = 1.0 - source.a;

        let result_r = lighten_r_premult + destination.r * inv_source_a;
        let result_g = lighten_g_premult + destination.g * inv_source_a;
        let result_b = lighten_b_premult + destination.b * inv_source_a;
