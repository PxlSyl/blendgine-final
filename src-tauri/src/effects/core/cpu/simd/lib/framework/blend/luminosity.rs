use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        luminosity::LuminosityBlend as FallbackLuminosityBlend,
    },
...
pub struct LuminosityBlend {
    fallback: FallbackLuminosityBlend,
}

impl LuminosityBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackLuminosityBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for LuminosityBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                ...
                let (_, _, source_l) = blend_utils::rgb_to_hsl(&source_unpremult);
                let (dest_h, dest_s, _) = blend_utils::rgb_to_hsl(&dest_unpremult);

                let (blend_r, blend_g, blend_b) = blend_utils::hsl_to_rgb(dest_h, dest_s, source_l);
