use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        difference::DifferenceBlend as FallbackDifferenceBlend,
    },
...
pub struct DifferenceBlend {
    fallback: FallbackDifferenceBlend,
}

impl DifferenceBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackDifferenceBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for DifferenceBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let sr_minus_dr = A::sub_ps(&sr_unpremult, &dr_unpremult);
        let sg_minus_dg = A::sub_ps(&sg_unpremult, &dg_unpremult);
        let sb_minus_db = A::sub_ps(&sb_unpremult, &db_unpremult);

        let dr_minus_sr = A::sub_ps(&dr_unpremult, &sr_unpremult);
        let dg_minus_sg = A::sub_ps(&dg_unpremult, &sg_unpremult);
        let db_minus_sb = A::sub_ps(&db_unpremult, &sb_unpremult);

        let difference_r = A::max_ps(&sr_minus_dr, &dr_minus_sr);
        let difference_g = A::max_ps(&sg_minus_dg, &dg_minus_sg);
        let difference_b = A::max_ps(&sb_minus_db, &db_minus_sb);

        let difference_r_premult = A::mul_ps(&difference_r, &sa);
        let difference_g_premult = A::mul_ps(&difference_g, &sa);
        let difference_b_premult = A::mul_ps(&difference_b, &sa);
        ...
        let result_r_premult = A::add_ps(&difference_r_premult, &dr_scaled);
        let result_g_premult = A::add_ps(&difference_g_premult, &dg_scaled);
        let result_b_premult = A::add_ps(&difference_b_premult, &db_scaled);
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                let r_val = A::extract_f32(&difference_r, i);
                let g_val = A::extract_f32(&difference_g, i);
                let b_val = A::extract_f32(&difference_b, i);
