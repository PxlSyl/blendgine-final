use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        overlay::OverlayBlend as FallbackOverlayBlend,
    },
...
pub struct OverlayBlend {
    fallback: FallbackOverlayBlend,
}

impl OverlayBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackOverlayBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for OverlayBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let double_mult_r = A::mul_ps(&two, &A::mul_ps(&sr_unpremult, &dr_unpremult));
        let double_mult_g = A::mul_ps(&two, &A::mul_ps(&sg_unpremult, &dg_unpremult));
        let double_mult_b = A::mul_ps(&two, &A::mul_ps(&sb_unpremult, &db_unpremult));

        let inv_sr_unpremult = A::sub_ps(&one, &sr_unpremult);
        let inv_sg_unpremult = A::sub_ps(&one, &sg_unpremult);
        let inv_sb_unpremult = A::sub_ps(&one, &sb_unpremult);

        let inv_dr_unpremult = A::sub_ps(&one, &dr_unpremult);
        let inv_dg_unpremult = A::sub_ps(&one, &dg_unpremult);
        let inv_db_unpremult = A::sub_ps(&one, &db_unpremult);

        let double_screen_r = A::sub_ps(
            &one,
            &A::mul_ps(&two, &A::mul_ps(&inv_sr_unpremult, &inv_dr_unpremult)),
        );
        let double_screen_g = A::sub_ps(
            &one,
            &A::mul_ps(&two, &A::mul_ps(&inv_sg_unpremult, &inv_dg_unpremult)),
        );
        let double_screen_b = A::sub_ps(
            &one,
            &A::mul_ps(&two, &A::mul_ps(&inv_sb_unpremult, &inv_db_unpremult)),
        );

        let mask_r = A::cmp_lt_ps(&dr_unpremult, &half);
        let mask_g = A::cmp_lt_ps(&dg_unpremult, &half);
        let mask_b = A::cmp_lt_ps(&db_unpremult, &half);

        let overlay_r_unpremult = A::or_ps(
            &A::and_ps(&mask_r, &double_mult_r),
            &A::andnot_ps(&mask_r, &double_screen_r),
        );
        let overlay_g_unpremult = A::or_ps(
            &A::and_ps(&mask_g, &double_mult_g),
            &A::andnot_ps(&mask_g, &double_screen_g),
        );
        let overlay_b_unpremult = A::or_ps(
            &A::and_ps(&mask_b, &double_mult_b),
            &A::andnot_ps(&mask_b, &double_screen_b),
        );

        let overlay_r_premult = A::mul_ps(&overlay_r_unpremult, &sa);
        let overlay_g_premult = A::mul_ps(&overlay_g_unpremult, &sa);
        let overlay_b_premult = A::mul_ps(&overlay_b_unpremult, &sa);

        let dr_scaled = A::mul_ps(&dr, &inv_sa);
        let dg_scaled = A::mul_ps(&dg, &inv_sa);
        let db_scaled = A::mul_ps(&db, &inv_sa);

        let result_r_premult = A::add_ps(&overlay_r_premult, &dr_scaled);
        let result_g_premult = A::add_ps(&overlay_g_premult, &dg_scaled);
        let result_b_premult = A::add_ps(&overlay_b_premult, &db_scaled);
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                let r_val = A::extract_f32(&overlay_r_unpremult, i);
                let g_val = A::extract_f32(&overlay_g_unpremult, i);
                let b_val = A::extract_f32(&overlay_b_unpremult, i);
