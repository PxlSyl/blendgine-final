use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        destination_atop::DestinationAtopBlend as FallbackDestinationAtopBlend,
    },
...
pub struct DestinationAtopBlend {
    fallback: FallbackDestinationAtopBlend,
}

impl DestinationAtopBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackDestinationAtopBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for DestinationAtopBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let blend_r = A::add_ps(
            &A::mul_ps(&dr_unpremult, &da),
            &A::mul_ps(&sr_unpremult, &inv_da),
        );
        let blend_g = A::add_ps(
            &A::mul_ps(&dg_unpremult, &da),
            &A::mul_ps(&sg_unpremult, &inv_da),
        );
        let blend_b = A::add_ps(
            &A::mul_ps(&db_unpremult, &da),
            &A::mul_ps(&sb_unpremult, &inv_da),
        );

        let result_r = A::mul_ps(&blend_r, &sa);
        let result_g = A::mul_ps(&blend_g, &sa);
        let result_b = A::mul_ps(&blend_b, &sa);
