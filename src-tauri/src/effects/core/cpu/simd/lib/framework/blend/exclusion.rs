use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        exclusion::ExclusionBlend as FallbackExclusionBlend,
    },
...
pub struct ExclusionBlend {
    fallback: FallbackExclusionBlend,
}

impl ExclusionBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackExclusionBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for ExclusionBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let product_r = A::mul_ps(&sr_unpremult, &dr_unpremult);
        let product_g = A::mul_ps(&sg_unpremult, &dg_unpremult);
        let product_b = A::mul_ps(&sb_unpremult, &db_unpremult);

        let double_product_r = A::mul_ps(&two, &product_r);
        let double_product_g = A::mul_ps(&two, &product_g);
        let double_product_b = A::mul_ps(&two, &product_b);

        let sum_r = A::add_ps(&sr_unpremult, &dr_unpremult);
        let sum_g = A::add_ps(&sg_unpremult, &dg_unpremult);
        let sum_b = A::add_ps(&sb_unpremult, &db_unpremult);

        let exclusion_r = A::sub_ps(&sum_r, &double_product_r);
        let exclusion_g = A::sub_ps(&sum_g, &double_product_g);
        let exclusion_b = A::sub_ps(&sum_b, &double_product_b);

        let exclusion_r_premult = A::mul_ps(&exclusion_r, &sa);
        let exclusion_g_premult = A::mul_ps(&exclusion_g, &sa);
        let exclusion_b_premult = A::mul_ps(&exclusion_b, &sa);
        ...
        let result_r_premult = A::add_ps(&exclusion_r_premult, &dr_scaled);
        let result_g_premult = A::add_ps(&exclusion_g_premult, &dg_scaled);
        let result_b_premult = A::add_ps(&exclusion_b_premult, &db_scaled);
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                let r_val = A::extract_f32(&exclusion_r, i);
                let g_val = A::extract_f32(&exclusion_g, i);
                let b_val = A::extract_f32(&exclusion_b, i);
