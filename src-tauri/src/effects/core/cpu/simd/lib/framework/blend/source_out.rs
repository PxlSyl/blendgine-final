use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        source_out::SourceOutBlend as FallbackSourceOutBlend,
    },
...
pub struct SourceOutBlend {
    fallback: FallbackSourceOutBlend,
}

impl SourceOutBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackSourceOutBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for SourceOutBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let result_a = A::mul_ps(&sa, &inv_da);
