use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        destination_out::DestinationOutBlend as FallbackDestinationOutBlend,
    },
...
pub struct DestinationOutBlend {
    fallback: FallbackDestinationOutBlend,
}

impl DestinationOutBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackDestinationOutBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for DestinationOutBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let (_, _, _, sa_raw) = A::load_rgba_with_alpha(source, x_base, y, 0, 0);
        ...
        let result_a = A::mul_ps(&da, &inv_sa);

        let result_r = A::mul_ps(&dr_unpremult, &result_a);
        let result_g = A::mul_ps(&dg_unpremult, &result_a);
        let result_b = A::mul_ps(&db_unpremult, &result_a);
