use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        xor::XorBlend as FallbackXorBlend,
    },
...
pub struct XorBlend {
    fallback: FallbackXorBlend,
}

impl XorBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackXorBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for XorBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let inv_da = A::sub_ps(&one, &da);

        let src_contrib_r = A::mul_ps(&sr_unpremult, &A::mul_ps(&sa, &inv_da));
        let src_contrib_g = A::mul_ps(&sg_unpremult, &A::mul_ps(&sa, &inv_da));
        let src_contrib_b = A::mul_ps(&sb_unpremult, &A::mul_ps(&sa, &inv_da));

        let dst_contrib_r = A::mul_ps(&dr_unpremult, &A::mul_ps(&da, &inv_sa));
        let dst_contrib_g = A::mul_ps(&dg_unpremult, &A::mul_ps(&da, &inv_sa));
        let dst_contrib_b = A::mul_ps(&db_unpremult, &A::mul_ps(&da, &inv_sa));

        let result_r_unpremult = A::add_ps(&src_contrib_r, &dst_contrib_r);
        let result_g_unpremult = A::add_ps(&src_contrib_g, &dst_contrib_g);
        let result_b_unpremult = A::add_ps(&src_contrib_b, &dst_contrib_b);

        let sa_da_product = A::mul_ps(&sa, &da);
        let double_product = A::mul_ps(&two, &sa_da_product);
        let result_a = A::sub_ps(&A::add_ps(&sa, &da), &double_product);
        ...
        for i in 0..A::chunk_size() {
            ...
            let r_val = A::extract_f32(&result_r_unpremult, i);
            let g_val = A::extract_f32(&result_g_unpremult, i);
            let b_val = A::extract_f32(&result_b_unpremult, i);
