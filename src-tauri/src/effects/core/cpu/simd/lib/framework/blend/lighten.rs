use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        lighten::LightenBlend as FallbackLightenBlend,
    },
...
pub struct LightenBlend {
    fallback: FallbackLightenBlend,
}

impl LightenBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackLightenBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for LightenBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let lighten_r_unpremult = A::max_ps(&sr_unpremult, &dr_unpremult);
        let lighten_g_unpremult = A::max_ps(&sg_unpremult, &dg_unpremult);
        let lighten_b_unpremult = A::max_ps(&sb_unpremult, &db_unpremult);

        let lighten_r_premult = A::mul_ps(&lighten_r_unpremult, &sa);
        let lighten_g_premult = A::mul_ps(&lighten_g_unpremult, &sa);
        let lighten_b_premult = A::mul_ps(&lighten_b_unpremult, &sa);

        let dr_scaled = A::mul_ps(&dr, &inv_sa);
        let dg_scaled = A::mul_ps(&dg, &inv_sa);
        let db_scaled = A::mul_ps(&db, &inv_sa);

        let result_r_premult = A::add_ps(&lighten_r_premult, &dr_scaled);
        let result_g_premult = A::add_ps(&lighten_g_premult, &dg_scaled);
        let result_b_premult = A::add_ps(&lighten_b_premult, &db_scaled);
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                let r_val = A::extract_f32(&lighten_r_unpremult, i);
                let g_val = A::extract_f32(&lighten_g_unpremult, i);
                let b_val = A::extract_f32(&lighten_b_unpremult, i);
