use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        destination_over::DestinationOverBlend as FallbackDestinationOverBlend,
    },
...
pub struct DestinationOverBlend {
    fallback: FallbackDestinationOverBlend,
}

impl DestinationOverBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackDestinationOverBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for DestinationOverBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let weighted_src_r = A::mul_ps(&sr_unpremult, &A::mul_ps(&sa, &inv_da));
        let weighted_src_g = A::mul_ps(&sg_unpremult, &A::mul_ps(&sa, &inv_da));
        let weighted_src_b = A::mul_ps(&sb_unpremult, &A::mul_ps(&sa, &inv_da));

        let weighted_dst_r = A::mul_ps(&dr_unpremult, &da);
        let weighted_dst_g = A::mul_ps(&dg_unpremult, &da);
        let weighted_dst_b = A::mul_ps(&db_unpremult, &da);

        let blend_r = A::add_ps(&weighted_dst_r, &weighted_src_r);
        let blend_g = A::add_ps(&weighted_dst_g, &weighted_src_g);
        let blend_b = A::add_ps(&weighted_dst_b, &weighted_src_b);

        let result_a = A::add_ps(&da, &A::mul_ps(&sa, &inv_da));

        let result_a_for_div = A::max_ps(&result_a, &epsilon);
        let result_r = A::div_ps(&blend_r, &result_a_for_div);
        let result_g = A::div_ps(&blend_g, &result_a_for_div);
        let result_b = A::div_ps(&blend_b, &result_a_for_div);
