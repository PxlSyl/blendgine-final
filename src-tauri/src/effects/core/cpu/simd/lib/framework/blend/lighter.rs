use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        lighter::LighterBlend as FallbackLighterBlend,
    },
...
pub struct LighterBlend {
    fallback: FallbackLighterBlend,
}

impl LighterBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackLighterBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for LighterBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let result_r_premult = A::add_ps(&sr, &dr);
        let result_g_premult = A::add_ps(&sg, &dg);
        let result_b_premult = A::add_ps(&sb, &db);

        let result_r_clamped = A::min_ps(&result_r_premult, &one);
        let result_g_clamped = A::min_ps(&result_g_premult, &one);
        let result_b_clamped = A::min_ps(&result_b_premult, &one);

        let sa_da_product = A::mul_ps(&sa, &da);
        let result_a = A::sub_ps(&A::add_ps(&sa, &da), &sa_da_product);
        let result_a_clamped = A::min_ps(&result_a, &one);

        let epsilon = A::set1_ps(1e-6);
        let result_a_for_div = A::max_ps(&result_a_clamped, &epsilon);
        let final_r = A::div_ps(&result_r_clamped, &result_a_for_div);
        let final_g = A::div_ps(&result_g_clamped, &result_a_for_div);
        let final_b = A::div_ps(&result_b_clamped, &result_a_for_div);
        ...
        for i in 0..A::chunk_size() {
            ...
            let r_val = A::extract_f32(&final_r, i);
            let g_val = A::extract_f32(&final_g, i);
            let b_val = A::extract_f32(&final_b, i);
