use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        hue::HueBlend as FallbackHueBlend,
    },
...
pub struct HueBlend {
    fallback: FallbackHueBlend,
}

impl HueBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackHueBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for HueBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                ...
                let (source_h, _, _) = blend_utils::rgb_to_hsl(&source_unpremult);
                let (_, dest_s, dest_l) = blend_utils::rgb_to_hsl(&dest_unpremult);

                let (blend_r, blend_g, blend_b) = blend_utils::hsl_to_rgb(source_h, dest_s, dest_l);
