use crate::effects::core::cpu::simd::{
    fallback::blend::{
        ...
        hard_light::HardLightBlend as FallbackHardLightBlend,
    },
...
pub struct HardLightBlend {
    fallback: FallbackHardLightBlend,
}

impl HardLightBlend {
    pub fn new() -> Self {
        Self {
            fallback: FallbackHardLightBlend::new(),
        }
    }
}

impl<A: SimdArchitecture + Sync> BlendProcessor<A> for HardLightBlend {
    ...
    unsafe fn process_simd_chunk(
        ...
        let double_sr = A::mul_ps(&two, &sr_unpremult);
        let double_sg = A::mul_ps(&two, &sg_unpremult);
        let double_sb = A::mul_ps(&two, &sb_unpremult);

        let mult_r = A::mul_ps(&double_sr, &dr_unpremult);
        let mult_g = A::mul_ps(&double_sg, &dg_unpremult);
        let mult_b = A::mul_ps(&double_sb, &db_unpremult);
        ...
        let double_inv_sr = A::mul_ps(&two, &inv_sr);
        let double_inv_sg = A::mul_ps(&two, &inv_sg);
        let double_inv_sb = A::mul_ps(&two, &inv_sb);

        let double_mult_inv_r = A::mul_ps(&double_inv_sr, &inv_dr);
        let double_mult_inv_g = A::mul_ps(&double_inv_sg, &inv_dg);
        let double_mult_inv_b = A::mul_ps(&double_inv_sb, &inv_db);

        let screen_r = A::sub_ps(&one, &double_mult_inv_r);
        let screen_g = A::sub_ps(&one, &double_mult_inv_g);
        let screen_b = A::sub_ps(&one, &double_mult_inv_b);
        ...
        let hard_light_r = A::or_ps(
            &A::and_ps(&src_mask_r, &mult_r),
            &A::andnot_ps(&src_mask_r, &screen_r),
        );
        let hard_light_g = A::or_ps(
            &A::and_ps(&src_mask_g, &mult_g),
            &A::andnot_ps(&src_mask_g, &screen_g),
        );
        let hard_light_b = A::or_ps(
            &A::and_ps(&src_mask_b, &mult_b),
            &A::andnot_ps(&src_mask_b, &screen_b),
        );

        let hard_light_r_premult = A::mul_ps(&hard_light_r, &sa);
        let hard_light_g_premult = A::mul_ps(&hard_light_g, &sa);
        let hard_light_b_premult = A::mul_ps(&hard_light_b, &sa);
        ...
        let result_r_premult = A::add_ps(&hard_light_r_premult, &dr_scaled);
        let result_g_premult = A::add_ps(&hard_light_g_premult, &dg_scaled);
        let result_b_premult = A::add_ps(&hard_light_b_premult, &db_scaled);
        ...
        for i in 0..A::chunk_size() {
            ...
            if src_a >= 0.9999 {
                let r_val = A::extract_f32(&hard_light_r, i);
                let g_val = A::extract_f32(&hard_light_g, i);
                let b_val = A::extract_f32(&hard_light_b, i);
