...
const GeneratingAnimationComponent: React.FC<GeneratingAnimationProps> = ({
  showDots,
  showConfetti,
  showSuccessScreen,
  onBackToMenu,
  ...
  const { isPaused, generationState, handleCancel, setShowMenu, isCancelling, setGenerationState } =
    useGenerateStore();
  ...
  useEffect(() => {
    ...
    setupListeners().catch(() => {
      // Error handling for setup listeners
    });
  }, [setShowMenu, setGenerationState]);

  const { sortedConsoleMessages } = useSortedData();
  const { handlePauseToggle } = useGenerationActions();
  const { currentProgress } = useGenerationProgress();
  ...
  const isProcessing = generationState.status === 'generating';
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="w-full max-w-md">
                ...
                {isPaused ? (
                  ...
                  <div className="mt-4 w-full py-2 px-4 bg-[rgb(var(--color-quaternary))] text-white font-semibold rounded-sm text-center">
                ...
                <div className="h-[40px] mt-8 flex justify-center">
                  {showDots && !isPaused && generationState.status !== 'cancelled' && (
          ...
          <div className="h-1/3 min-h-[250px] border-t border-gray-700">
            <GenerationConsole
              ...
              onPauseToggle={(isPaused) => {
                void handlePauseToggle(isPaused);
              }}
              isPaused={isPaused}
...
export default GeneratingAnimationComponent;
