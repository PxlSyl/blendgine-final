...
export const TabButton = React.memo<TabButtonProps>(
  ({ isSelected, onClick, icon, label, layoutId, disabled, sidebarFull }) => (
    <button
      onClick={onClick}
      disabled={disabled}
    ...
    >
      <div className="relative z-10 flex items-center h-full">
        ...
        <div className={`hidden ${sidebarFull ? 'lg:block' : ''} px-2`}>
          <span
            className={
              isSelected
                ? 'text-white'
      ...
      {isSelected && !disabled && (
        <motion.span
          ...
          className="rounded-sm absolute inset-0 z-0 bg-gradient-to-r from-[rgb(var(--color-secondary-light))] to-[rgb(var(--color-secondary))]"
          style={{
            boxShadow:
              'inset 0 1px 0 0 rgba(255,255,255,0.4), inset 0 -2px 2px rgba(0,0,0,0.2), inset -2px 0 2px rgba(0,0,0,0.2), 0 1px 3px rgba(0,0,0,0.2)',
